  ...
  struct CatPass : public FunctionPass {
    ...
    virtual bool runOnFunction(Function &F) {
        ...
        for (auto &I : B) {        
          ...
          switch (calleeID) {
            case CAT_binary_add_ID: {
              ...
              KILL_sets[&I].insert(dyn_cast<Instruction>(defineVal));
                  ...
                  if (setCalleeID(val_user_inst, &cid, &cinst)) {
                    if (cid == CAT_binary_add_ID) {
                      if (defineVal ==  cinst->getArgOperand(0)) {
                        KILL_sets[&I].insert(val_user_inst);
                      }                    }
                  }
              }
              auto search = KILL_sets[&I].find(&I);
              if (search != KILL_sets[&I].end()) {
                KILL_sets[&I].erase(search);
              }
              break;
            }
      ...
      for (inst_iterator iter = inst_begin(F), E = inst_end(F); iter != E; ++iter) {
        ...
        errs() << "{\n";
        if (!GEN_sets[&I].empty()) {
          for (auto inst = GEN_sets[&I].begin(); inst != GEN_sets[&I].end(); ++inst) {
            errs() << ' ' << **inst << '\n';
          }
        }
        errs() << "}\n";
        ...
        errs() << "***************** KILL\n";
        errs() << "{\n";
        if (!KILL_sets[&I].empty()) {
          for (auto inst = KILL_sets[&I].begin(); inst != KILL_sets[&I].end(); ++inst) {
            errs() << ' ' << **inst << '\n';
          }
        }
        errs() << "}\n";
